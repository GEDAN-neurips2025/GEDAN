...
from src.utils.dataset_load import getMol, getZinc
...
def getCouplePair(d1, d2, batch_size, model, S_GEDAN, sez, max_nodes):
    ...
    if S_GEDAN:
        np.save(f"results/{dataset_name}/encoding/dati_SGEDAN_{sez}.npy", np.array(ged_tmp))
        np.save(f"results/{dataset_name}/encoding/label_SGEDAN_{sez}.npy", np.array(label_))
    else:
        np.save(f"results/{dataset_name}/encoding/dati_UGEDAN_{sez}.npy", np.array(ged_tmp))
        np.save(f"results/{dataset_name}/encoding/label_UGEDAN_{sez}.npy", np.array(label_))
...
def getPivot(dataset_name):
    ...
    if dataset_name == "ZINC":
        GED_train, GED_test = getZinc(32, seed)
...
def run(dataset_name, S_GEDAN, GED_train, GED_test, pivots, device):
    ...
    if S_GEDAN:
        ...
        if dataset_name == "BBBP":
            model.load_state_dict(torch.load(f"src/checkpoints/ckp_BBBP.pt"))

        if dataset_name == "FreeSolv":
            model.load_state_dict(torch.load(f"src/checkpoints/ckp_FreeSolv.pt"), strict=False)

        if dataset_name == "ZINC":
            model.load_state_dict(torch.load(f"src/checkpoints/ckp_ZINC.pt"), strict=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, help="Dataset [BBBP, FreeSolv, ZINC]")

    args = parser.parse_args()

    dataset_name = args.dataset

    if dataset_name not in ["ZINC", "FreeSolv", "BBBP"]:
