...
from torch_geometric.datasets import TUDataset, MoleculeNet, ZINC
...
# Get ZINC dataset
def getZinc(limit, seed=0):

    dataset = ZINC("~/Dataset", subset=True)

    data_ = []
    for i in range(len(dataset)):
        data_.append(Data(x=dataset[i].x, edge_index=dataset[i].edge_index, y=dataset[i].y.unsqueeze(1)))
