    ...
    def geMatrixCost(self, diff_node_, edge1, edge2, mask):
        ...
        if self.learned_cost:
            edge = (edge1 * F.softplus(self.weight_edge1, beta=5) +
                    edge2 * F.softplus(self.weight_edge2, beta=5)).transpose(1, 2)

            matrix_sub = ((edge + (diff_node_ * F.softplus(self.weight_sub1, beta=5)))
                          * F.softplus(self.weight_sub2, beta=5)) * mask_sub.transpose(1, 2)
            ...
            edge = (edge1 *self.weight_edge1 +
                    edge2 *self.weight_edge2).transpose(1, 2)

            matrix_sub = ((edge + (diff_node_ * self.weight_sub1))
                          * self.weight_sub2) * mask_sub.transpose(1, 2)
        ...
        all_matrix = ((all_matrix.max() + F.relu(self.w_max)) * mask_pad_) + all_matrix
