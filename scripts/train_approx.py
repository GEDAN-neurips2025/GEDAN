...
def train_GED(dataset_n, supervised, idx_cost):
    ...
    for train_index, test_index in KFold(n_splits=5, shuffle=True).split(data_graph):
        ...
        d_train = DataLoader(graphs_train, batch_size=config_model["bs_train"], shuffle=True)
        d_val = DataLoader(graphs_val, batch_size=config_model["bs_test"], shuffle=False)
